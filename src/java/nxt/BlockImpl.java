    ...
    void apply() {
        ...
        if (this.height > Constants.SHUFFLING_BLOCK) {
            ...
            for (int i = 0; i < backFees.length; i++) {
                ...
                Logger.logDebugMessage("Back fees %f WeOneCoin ( WOC ) to forger at height %d", ((double)backFees[i])/Constants.ONE_NXT, this.height - i - 1);
        ...
        if (totalBackFees != 0) {
            Logger.logDebugMessage("Fee reduced by %f WeOneCoin ( WOC ) at height %d", ((double)totalBackFees)/Constants.ONE_NXT, this.height);
        }
