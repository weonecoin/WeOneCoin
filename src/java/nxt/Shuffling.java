    ...
    private void cancel(Block block) {
        ...
        if (blamedAccountId != 0) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                Logger.logDebugMessage("Shuffling penalty %f WeOneCoin ( WOC ) awarded to forger at height %d", ((double)fee) / Constants.ONE_NXT, block.getHeight() - i - 1);
            }
            ...
            Logger.logDebugMessage("Shuffling penalty %f WeOneCoin ( WOC ) awarded to forger at height %d", ((double)fee) / Constants.ONE_NXT, block.getHeight());
        }
