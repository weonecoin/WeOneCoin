...
var NRS = (function(NRS, $) {
    ...
    NRS.submitForm = function($modal, $btn) {
		...
		$form.find(":input").each(function() {
			if ($(this).is(":invalid")) {
				var error = "";
				var name = String($(this).attr("name")).replace("WOC", "").replace("WOC", "").capitalize();
				var value = $(this).val();
		...
		if (data.recipient) {
			...
			if (/^\d+$/.test(data.recipient)) {
				...
				NRS.unlockForm($modal, $btn);
				return;
			} else if (!/^WOC\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(data.recipient)) {
				var convertedAccountId = $modal.find("input[name=converted_account_id]").val();
				if (!convertedAccountId || (!/^\d+$/.test(convertedAccountId) && !/^WOC\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(convertedAccountId))) {
