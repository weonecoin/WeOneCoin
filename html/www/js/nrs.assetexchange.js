...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.forms.addAssetBookmark = function ($modal) {
        ...
        if (!/^\d+$/.test(data.id) && !/^WOC\-/i.test(data.id)) {
            return {
                "error": $.t("error_asset_or_account_id_invalid")
            };
        }

        if (/^WOC\-/i.test(data.id)) {
            NRS.sendRequest("getAssetsByIssuer", {
                "account": data.id
            }, function (response) {
                if (response.errorCode) {
    ...
    $("#asset_order_modal").on("show.bs.modal", function (e) {
        ...
        $("#asset_order_total").html(totalNXT + " WeOneCoin ( WOC )");
    ...
    NRS.setup.asset_exchange = function () {
        ...
        NRS.appendMenuItemToTSMenuItem(sidebarId, options);
        options = {
            "titleHTML": '<span data-i18n="page1_page">Exchange Trading</span>',
            "type": 'PAGE',
            "page": 'page1'
        };
        NRS.appendMenuItemToTSMenuItem(sidebarId, options);
